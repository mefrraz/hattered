document.addEventListener('DOMContentLoaded', () => {
    ...
    function parseGuides(text) {
        ...
        return lines.map((line, index) => {
            // NOVA LINHA: Remove o prefixo e qualquer espaço extra
            const cleanedLine = line.replace(/^\s*/, '').trim(); 
            const parts = cleanedLine.split('|');

            // Verifica se a linha tem pelo menos 5 partes esperadas
            if (parts.length >= 5) {
                return {
                    id: `post-${index}`,
                    category: parts[0].trim(), // Agora parts[0] já não terá o prefixo 
                    link: parts[1].trim(),
                    title: parts[2].trim(),
                ...
                console.warn('Linha inválida ou incompleta ignorada:', line);
                return null;
            }
    ...
    function renderMorePosts(posts, postsShownOnTop) {
        ...
        // NOVO: Aleatoriza a ordem dos posts restantes
